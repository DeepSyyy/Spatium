...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  Future<void> _showMoodPicker() async {
    ...
    if (result != null && mounted) {
      ...
      final success = await ref.read(homeNotifierProvider.notifier).saveTodayMood(moodId, note);
      
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(success ? 'Mood berhasil disimpan!' : 'Gagal menyimpan mood'),
        ...
        children: [
          ...
          if (chats.isNotEmpty) ...[
            ...
            ...chats.map((chat) => Container(
              ...
              child: Row(
                children: [
                  Icon(Icons.chat_bubble_outline, color: AppColor.primary, size: 20),
                  const SizedBox(width: AppConstants.spacingM),
                  Expanded(
                    child: Text(
                      chat.title,
